...
We may at times need to introduce additional parties. For the sake of simplicity, all evil attackers will use names that start with an E and all opponents with start with an O. All other actors will draw from the remainder of the alphabet.
...
* `Cryptographic Information (plain/cipher texts, keys, ...) is represented by inline code`
...
A finite set simply means that the number of elements within the set cannot be infinite (which makes sense, as there's only so many ways to rearrange a number of symbols to form a text). Usually, the above criteria are represented by symbols grouped into something called a tuple, so we don't have to write the entire list each time. For these posts we'll use the following symbols:
...
Let's break this down. What it basically says (in math terms) is that each key must be made in a way that allows you to encrypt and decrypt with it. "Encryption" is a function that creates a cipher text out of a plain text, "decryption" goes the other way around and creates _a_ plain text from a cipher text. And to be useful for its intended purpose, applying a decryption function on a text that has been encrypted using an encryption function from the same crypto system _must_ restore the original plain text.
...
The key in this case is the amount of letters you shift. Assuming that `A` can be represented by the number `1`, `B` by the number `2` and so on (you can quickly see where this is going), you can for example use a key of 5 to tell the recipient of your message that you've replaced all `A`s (`1`) with `F`s (`6`), all `B`s with `G`s, etc...
...
> $K$ is a number that specifies the shift (I'll dive into this a in a bit)
> $e_k$ is shifting the alphabet by a number of steps defined by $K$
> $d_k$ is shifting the alphabet by a number of stept defined as $-K$ (to invert e_k)


All that being said, what if we were able to come up with a crypto system that
...
This is where we switch over to

#### Asymmetrical crypto systems
These are also known as "public key systems" (a term you might have heard before). The main advantage they have over symmetrical systems is that they allow you to encrypt your communication without ever sharing your encryption key (also called "private key") with anyone. All you need to do is share the namesake public key (which as we'll see is linked to the private key by some pretty interesting math) and your conversation partners will be able to infer the decryption rule from your message and the key they know. On the downside, the required math makes it a little harder to calculate the keys, and you need bigger/longer keys to reach the same level of security.
